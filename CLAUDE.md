...
├── tests/                           # Unit tests (552) — no hardware required
...
- **⚠️ The I/E range was never set until 2026-09-25 — read
  [`docs/gamry-current-range-2026-09-25.md`](docs/gamry-current-range-2026-09-25.md).**
  `initialize_pstat()` configured nine hardware parameters and not the current range, so
  every Python-mode run sat on the instrument's power-up range, **`IERange 11` = 600 mA**,
...
`data.py`; hardware is faked (`fakes.py`) so all 552 tests run with no instruments attached.
...
- **`gui/` coverage — no longer the gap it was.** 552 tests total (550 pass, 2 skip; counted 2026-09-25);
  `tests/test_gui_layout.py` alone holds 153 and `tests/test_dark_save.py` another 4, both
